...
InterfaceKevin::InterfaceKevin(list<Route>* ipTable, map<bool, queue<Message> >* messagePool, char* network, char* ipAddress, char* macAddress, char* realIpAddress, int realPort, char* dispatcherAddress, int dispatcherPort){
    this->empty = false;
...
void InterfaceKevin::wakeUp(char* macAddress, char* realIpAddress, int dispatcherPort, char* dispatcherAddress){
    Socket s;
    char buffer[ 512 ] = "";
...
void InterfaceKevin::receive(){ //nice to have: dos hilos
	int childpid;
    int status = 0;
...
Message InterfaceKevin::checkSharedMemory() {
...
void InterfaceKevin::addToSharedMemory(Message message){
    if((string)this->macAddress=="Bolinchas.Kevin"){
        mutx.lock();
        map<bool,queue<Message> >::iterator it = messagePool->find(true);
        mutx.unlock();        
...
Envelope InterfaceKevin::assemblePackage(char* message){
...
void InterfaceKevin::processEnvelope(){
        if(!this->inbox.empty()) {
...
bool InterfaceKevin::isBroadcast(char* macReceiver){
    return (string)macReceiver == "*";

}

//este metodo devuelve la distancia que un broadcast pregunta
int InterfaceKevin::getDistance(char* ipAddress){
...
char* InterfaceKevin::checkIpTable(char* ipAddress){
    char* network = this->getNetwork(ipAddress);
    mutx.lock();
    list<Route>::iterator ite = this->ipTable->begin();
    while(ite!=this->ipTable->end() && *ite->getNetwork()!=*ipAddress){
...
char* InterfaceKevin::getNetwork(char* ipAddress){
...
bool InterfaceKevin::isSharedMemoryEmpty(){
    if((string)this->macAddress=="Bolinchas.Kevin"){
        mutx.lock();
        map<bool,queue<Message> >::iterator it = messagePool->find(false);
        mutx.unlock();
...
void InterfaceKevin::processMessage(){
    //while(true){
